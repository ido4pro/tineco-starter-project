...
void OtaUpdate(OtaDelegate _stopBeforeOta,OtaDelegate _restartAfterOta) {

	stopBeforeOta = _stopBeforeOta;
	restartAfterOta = _restartAfterOta;
	...
	debugf("otaUpdater->start ... slot:%d",slot);
	// start update
	otaUpdater->start();
}
...
void Switch() {
	...
	if (before == 0)
		...
		after = 0;
	debugf("Swapping from rom %d to rom %d.\r\n", before, after);
